...
const
  ...
  MAX_OBJECT_SIZE = 1024 * 1024 * 1024;  //最大对象大小 200M , 大于200M 则会认为错误的包。
  //BUF_BLOCK_SIZE = 1024 * 8;  // 发送数据时块大小
  MAX_BLOCK_SIZE = 1024 * 100;   // 100K
...
type
  ...
  {$IFDEF UNICODE}
    FChildren: TObjectList<TUPMsgPack>;
  {$ELSE}
    FChildren: TObjectList;
  {$ENDIF}
    procedure InnerAddToChildren(pvDataType: TMsgPackType; obj: TUPMsgPack);
    function InnerAdd(pvDataType: TMsgPackType): TUPMsgPack; overload;
    function InnerAdd:TUPMsgPack; overload;
  ...
  private
    ...
    function FindObj(pvName:string): TUPMsgPack;
  ...
  private
    ...
    /// </summary>
    function InnerFindPathObject(pvPath: string; var vParent: TUPMsgPack; var
        vIndex: Integer): TUPMsgPack;

    function GetO(pvPath: String): TUPMsgPack;
    procedure SetO(pvPath: String; const Value: TUPMsgPack);
    ...
    function GetItems(AIndex: Integer): TUPMsgPack;
  ...
  public
    ...
    function Add(pvNameKey, pvValue: string): TUPMsgPack; overload;
    function Add(pvNameKey: string; pvValue: Int64): TUPMsgPack; overload;
    function Add(pvNameKey: string; pvValue: TBytes): TUPMsgPack; overload;
    function Add(pvNameKey: String): TUPMsgPack; overload;
    function Add: TUPMsgPack; overload;

    function AddArrayChild: TUPMsgPack;

    function ForcePathObject(pvPath: string): TUPMsgPack;
    ...
    property O[pvPath: String]: TUPMsgPack read GetO write SetO;
    ...
    property Items[AIndex: Integer]: TUPMsgPack read GetItems; default;
...
///  copy from qmsgpack
/// </summary>
procedure writeArray(obj:TUPMsgPack; pvStream:TStream);
var
  c, i:Integer;
  lvValue:TMsgPackValue;
  lvNode:TUPMsgPack;
begin
  ...
  for I := 0 to C-1 do
  begin
    lvNode:=TUPMsgPack(obj.FChildren[I]);
    lvNode.InnerEncodeToStream(pvStream);
  end;
end;

procedure writeMap(obj:TUPMsgPack; pvStream:TStream);
var
  c, i:Integer;
  lvValue:TMsgPackValue;
  lvNode:TUPMsgPack;
...
constructor TUPMsgPack.Create;
begin
  inherited Create;
  {$IFDEF UNICODE}
    FChildren := TObjectList<TUPMsgPack>.Create(true);
  {$ELSE}
    FChildren := TObjectList.Create(true);
  {$ENDIF}
end;

procedure TUPMsgPack.DecodeFromBytes(pvBytes: TBytes);
var
  lvStream:TStream;
begin
...
procedure TUPMsgPack.DecodeFromFile(pvFileName: string);
var
...
procedure TUPMsgPack.DecodeFromStream(pvStream: TStream);
begin
  InnerParseFromStream(pvStream);
end;

function TUPMsgPack.DeleteObject(pvPath: String): Boolean;
var
  lvParent, lvObj:TUPMsgPack;
  j:Integer;
begin
...
function TUPMsgPack.Add(pvNameKey, pvValue: string): TUPMsgPack;
begin
...
function TUPMsgPack.Add(pvNameKey: string; pvValue: Int64): TUPMsgPack;
begin
  Result := InnerAdd(mptMap);
  Result.setName(pvNameKey);
...
function TUPMsgPack.Add: TUPMsgPack;
begin
  Result := InnerAdd(mptMap);
end;

function TUPMsgPack.AddArrayChild: TUPMsgPack;
begin
...
function TUPMsgPack.Add(pvNameKey: string; pvValue: TBytes): TUPMsgPack;
begin
  Result := InnerAdd(mptMap);
  Result.setName(pvNameKey);
...
function TUPMsgPack.Add(pvNameKey:String): TUPMsgPack;
begin
  Result := InnerAdd(mptMap);
  Result.setName(pvNameKey);
end;

procedure TUPMsgPack.CheckObjectDataType(ANewType: TMsgPackType);
begin
...
function TUPMsgPack.EncodeToBytes: TBytes;
var
  lvStream:TStream;
begin
  lvStream := TMemoryStream.Create;
...
procedure TUPMsgPack.EncodeToFile(pvFileName: string);
var
  lvFileStream:TFileStream;
begin
  if FileExists(pvFileName) then
...
procedure TUPMsgPack.EncodeToStream(pvStream: TStream);
begin
  InnerEncodeToStream(pvStream);
end;

function TUPMsgPack.FindObj(pvName: string): TUPMsgPack;
var
  i:Integer;
begin
...
function TUPMsgPack.ForcePathObject(pvPath:string): TUPMsgPack;
var
  lvName: string;
  s: string;
  sPtr: PChar;
  lvTempObj, lvParent: TUPMsgPack;
  j: Integer;
begin
  Result := nil;
  s := pvPath;
  ...
  begin
    ...
    begin
      ...
      begin           // end
        ...
        if j <> -1 then
        begin
          Result := TUPMsgPack(lvParent.FChildren[j]);
        end
        else
        begin
...
function TUPMsgPack.GetAsBoolean: Boolean;
begin
...
function TUPMsgPack.GetAsBytes: TBytes;
begin
  Result := FValue;
end;

function TUPMsgPack.GetAsDateTime: TDateTime;
begin
...
function TUPMsgPack.GetAsDouble: Double;
begin
...
function TUPMsgPack.getAsInteger: Int64;
begin
  case FDataType of
...
function TUPMsgPack.GetAsSingle: Single;
begin
  if FDataType in [mptDouble, mptDateTime] then
    Result := PDouble(FValue)^
  else
...
function TUPMsgPack.getAsString: String;
var
  l:Cardinal;
begin
  Result := '';
...
///   copy from qdac3
/// </summary>
function TUPMsgPack.GetAsVariant: Variant;
var
  I: Integer;
    ...
    mptArray, mptMap:
      begin
        ...
        for I := 0 to Count - 1 do
          Result[I] := TUPMsgPack(FChildren[I]).AsVariant;
      end;
    mptBinary:
      BytesAsVariant;
  else
...
function TUPMsgPack.GetB(pvPath: String): Boolean;
var
  lvObj:TUPMsgPack;
begin
  lvObj := GetO(pvPath);
...
function TUPMsgPack.GetCount: Integer;
begin
  Result := FChildren.Count;
end;

function TUPMsgPack.GetD(pvPath: String): Double;
var
  lvObj:TUPMsgPack;
begin
  lvObj := GetO(pvPath);
...
function TUPMsgPack.GetDT(pvPath: String): TDateTime;
var
  lvObj:TUPMsgPack;
begin
  lvObj := GetO(pvPath);
...
function TUPMsgPack.GetI(pvPath: String): Int64;
var
  lvObj:TUPMsgPack;
begin
  lvObj := GetO(pvPath);
...
function TUPMsgPack.GetItems(AIndex: Integer): TUPMsgPack;
begin
  Result := TUPMsgPack(FChildren[AIndex]);
end;

function TUPMsgPack.GetO(pvPath: String): TUPMsgPack;
var
  lvParent:TUPMsgPack;
  j:Integer;
begin
...
function TUPMsgPack.GetResult: Boolean;
var
  lvObj:TUPMsgPack;
begin
...
function TUPMsgPack.GetS(pvPath: String): string;
var
  lvObj:TUPMsgPack;
begin
  lvObj := GetO(pvPath);
...
function TUPMsgPack.IndexOf(pvName:string): Integer;
begin
  Result := indexOfIgnoreSensitive(LowerCase(pvName));
end;

function TUPMsgPack.IndexOfCaseSensitive(pvName:string): Integer;
var
  i, l: Integer;
  lvObj:TUPMsgPack;
begin
...
function TUPMsgPack.IndexOfIgnoreSensitive(pvLowerCaseName: string):
    Integer;
var
  i, l: Integer;
  lvObj:TUPMsgPack;
...
function TUPMsgPack.InnerAdd(pvDataType: TMsgPackType): TUPMsgPack;
begin
  Result := TUPMsgPack.Create;
  Result.FDataType := mptUnknown;
  InnerAddToChildren(pvDataType, Result);
end;

function TUPMsgPack.InnerAdd: TUPMsgPack;
begin
...
procedure TUPMsgPack.InnerAddToChildren(pvDataType: TMsgPackType; obj:
    TUPMsgPack);
begin
...
procedure TUPMsgPack.InnerEncodeToStream(pvStream:TStream);
begin
  case FDataType of
...
function TUPMsgPack.InnerFindPathObject(pvPath: string; var vParent:
    TUPMsgPack; var vIndex: Integer): TUPMsgPack;
var
  lvName:string;
  s:string;
  sPtr:PChar;
...
procedure TUPMsgPack.InnerParseFromStream(pvStream: TStream);
var
  lvByte:Byte;
...
procedure TUPMsgPack.LoadBinaryFromFile(pvFileName:String);
var
  lvFileStream:TFileStream;
begin
  if FileExists(pvFileName) then
...
procedure TUPMsgPack.LoadBinaryFromStream(pvStream: TStream; pvLen:
    cardinal = 0);
begin
  FDataType := mptBinary;
  if pvLen = 0 then
...
procedure TUPMsgPack.SaveBinaryToFile(pvFileName: String);
var
  lvFileStream:TFileStream;
begin
  if FileExists(pvFileName) then
...
procedure TUPMsgPack.SaveBinaryToStream(pvStream: TStream);
begin
  pvStream.WriteBuffer(FValue[0], Length(FValue));
end;

procedure TUPMsgPack.SetAsBoolean(const Value: Boolean);
begin
...
procedure TUPMsgPack.SetAsBytes(const Value: TBytes);
begin
  FDataType := mptBinary;
  FValue := Value;
end;

procedure TUPMsgPack.SetAsDateTime(const Value: TDateTime);
begin
...
procedure TUPMsgPack.SetAsDouble(const Value: Double);
begin
  FDataType := mptDouble;
...
procedure TUPMsgPack.setAsInteger(pvValue: Int64);
begin
...
procedure TUPMsgPack.SetAsSingle(const Value: Single);
begin
  FDataType := mptSingle;
...
procedure TUPMsgPack.setAsString(pvValue: string);
begin
  FDataType := mptString;
  if SizeOf(Char) = 2 then
  begin
...
///   copy from qdac3
/// </summary>
procedure TUPMsgPack.SetAsVariant(const Value: Variant);
var
  I: Integer;
  AType: TVarType;
...
procedure TUPMsgPack.SetB(pvPath: String; const Value: Boolean);
var
  lvObj:TUPMsgPack;
begin
...
procedure TUPMsgPack.SetD(pvPath: String; const Value: Double);
var
  lvObj:TUPMsgPack;
begin
  lvObj := ForcePathObject(pvPath);
...
procedure TUPMsgPack.SetDT(pvPath: String; const Value: TDateTime);
var
  lvObj:TUPMsgPack;
begin
  lvObj := ForcePathObject(pvPath);
...
procedure TUPMsgPack.SetI(pvPath: String; const Value: Int64);
var
  lvObj:TUPMsgPack;
begin
  lvObj := ForcePathObject(pvPath);
...
procedure TUPMsgPack.setName(pvName: string);
begin
  FName := pvName;
  FLowerName := LowerCase(FName);
end;

procedure TUPMsgPack.SetO(pvPath: String; const Value: TUPMsgPack);
var
  lvName:String;
  s:String;
  sPtr:PChar;
  ...
  begin
    ...
    begin
      ...
      begin           // end
        ...
        if j <> -1 then
        begin
          lvTempObj := TUPMsgPack(lvParent.FChildren[j]);
...
procedure TUPMsgPack.SetResult(const Value: Boolean);
var
  lvObj:TUPMsgPack;
begin
...
procedure TUPMsgPack.SetS(pvPath: String; const Value: string);
var
  lvObj:TUPMsgPack;
begin
  lvObj := ForcePathObject(pvPath);
