...
(*
  ...
  *      be misrepresented as being the original software.
  ...
  ?ÿ - replaced with all small russian letters (Win1251)
...
{$ENDIF}
  ...
  public
    ...
    property InputString: RegExprString read GetInputString
      write SetInputString;
...
begin
  ...
  begin
    ...
    begin
      Result := Result + System.Copy(AStr, i0, i - i0) + '\' + AStr[i];
      i0 := i + 1;
    end;
    Inc(i);
  end;
...
function RegExprSubExpressions(const ARegExpr: AnsiString; ASubExprs: TStrings;
  AExtendedSyntax: boolean{$IFDEF D4_} = False{$ENDIF}): integer;
type
...
begin
  case AErrorID of
    ...
    reeNoInpitStringSpecified:
      Result := 'TRegExpr(exec): No Inpit String Specified';
...
{$ELSE}
  ...
  RusRangeHi = 'ÀÁÂÃÄÅ¨ÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞ?; RusRangeLoLow = '?; RusRangeLoHigh = 'ÿ';
RusRangeHiLow = '?; RusRangeHiHigh = '?;
{$ENDIF}
      ...
      begin
        ...
        begin
          if (PREOp(scan)^ = EXACTLY) and
            (StrLen(scan + REOpSz + RENextOffSz) >= Len) then
          begin
...
{$IFDEF UniCode} // ###0.935
              if (ord((regparse + 1)^) < 256) and
                (AnsiChar((regparse + 1)^) in ['d', 'D', 's', 'S', 'w', 'W'])
              then
              begin
{$ELSE}
...
{$IFDEF ComplexBraces}
    ...
    begin // ###0.940
      Result := Result + Format(' -> (%d) {%d,%d}',
        [(s - programm - (REOpSz + RENextOffSz)) + PRENextOff(s + 2 *
        REBracesArgSz)^, PREBracesArg(s)^, PREBracesArg(s + REBracesArgSz)^]);
