...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if vDeItem[TDeProp.TraceAdd] = '' then  // 新痕迹
          vDeItem[TDeProp.TraceAdd] := UserInfo.Name + '(' + UserInfo.ID + ') 添加 ' + FormatDateTime('YYYY-MM-DD HH:mm:SS', FServerInfo.DateTime);
      end;

      if TDeTraceStyle.cseDel in vDeItem.TraceStyles then
      begin
        if vDeItem[TDeProp.TraceDel] = '' then  // 新痕迹
          vDeItem[TDeProp.TraceDel] := UserInfo.Name + '(' + UserInfo.ID + ') 删除 ' + FormatDateTime('YYYY-MM-DD HH:mm:SS', FServerInfo.DateTime);
      end;
    end;
  end;
end;
